...
/// Resolve the best matching version from a JSON array of version strings.
///
/// `available_json` — JSON array, e.g. `["1.0.0","1.2.0","2.0.0"]`
/// `req`            — semver requirement, e.g. `"^1.0"`
#[wasm_bindgen]
...
pub fn sha256_tree_json(entries_json: &str) -> Result<String, JsValue> {
    ...
    let mut entries: Vec<(String, Vec<u8>)> = raw
        ...
        .map(|e| {
            use std::io::Read;
            let mut buf = Vec::new();
            // simple base64 decode via engine
...
fn base64_decode(s: &str) -> Result<Vec<u8>, String> {
    // Minimal base64 decode without pulling in the base64 crate.
    // Delegates to js-sys atob for now; swap with a pure-Rust impl if needed.
    use js_sys::Uint8Array;
    let decoded = js_sys::eval(&format!("atob('{}')", s))
        .map_err(|e| format!("{:?}", e))?;
    let s = decoded.as_string().ok_or_else(|| "atob returned non-string".to_string())?;
